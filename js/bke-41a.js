...
function checkWinner(player_num) {
    // Rij 0 - Cel 0, 1 en 2
    if(GAME_FIELD_ELEMENT[0].src.search(PLAYER_IMAGES[player_num]) >= 0
       && GAME_FIELD_ELEMENT[1].src.search(PLAYER_IMAGES[player_num]) >= 0
       && GAME_FIELD_ELEMENT[2].src.search(PLAYER_IMAGES[player_num]) >= 0 )
        return true;

    // Ik ga er standaard van uit dat niemand gewonnen heeft
    return false;
}
